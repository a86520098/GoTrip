	...
	CommandLineRunner carCommentRunner() {
		...
		return args -> {
			for (int i = 0; i < 75; i++) {
				...
				randRating += 2; //2~5
				Comment comment = new Comment("carRental", randItemId, "user" + randUserId, new Timestamp(System.currentTimeMillis()), randRating, contentArray[randRating], "show");
	...
	CommandLineRunner ticketCommentRunner() {
		...
		return args -> {
			for (int i = 0; i < 75; i++) {
				...
				int randRating = rand.nextInt(4); //0-3
				randRating += 2; //2~5
				Comment comment = new Comment("ticket", randItemId, "user" + randUserId, new Timestamp(System.currentTimeMillis()), randRating,  contentArray[randRating], "show");
				cService.save(comment);
			}
	...
	CommandLineRunner hotelCommentRunner() {
		...
		return args -> {
			for (int i = 0; i < 75; i++) {
				...
				int randRating = rand.nextInt(4); //0-3
				randRating += 2; //2~5
				Comment comment = new Comment("hotel", randItemId, "user" + randUserId, new Timestamp(System.currentTimeMillis()), randRating,  contentArray[randRating], "show");
				cService.save(comment);
			}
